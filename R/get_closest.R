...
get_closest <- function(station_id,single=F,dist=0.5,overlap=365L,max_st=10) {
  ...
  json <- getURL(paste0('http://10.65.26.5:8000/get-closest/',station_id,'?single=',substr(single,1,1),'&dist=',dist,'&overlap=',overlap,'&max_st=',max_st),.opts=list(cookiefile=get('.cookie_file',envir=.GlobalEnv)))
